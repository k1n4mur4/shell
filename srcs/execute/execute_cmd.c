...
/*   Updated: 2025/07/30 01:48:05 by kinamura         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	execute_simple_command(t_simple_command *simple)
{
	...
	if (!simple)
		return (0);
	/* Handle redirect-only commands (no words) */
	if (!simple->words)
	{
		...
		/* For redirect-only commands, just setup and restore redirections */
		restore_redirections(&backup);
		return (0);
	}
		...
		t_command_type type)
{
	...
	if (type == CM_AND)
	{
		...
		if (left_exit == 0)
		{
			if (right)
			{
				/* Expand right side now that left side has executed */
				expand_ast(right);
		...
		/* If left failed,
			return left's failure status without executing right */
	}
	else if (type == CM_OR)
	{
		...
		/* If left succeeded,
			return left's success status without executing right */
	}
	return (final_exit);
}
